...
class RollCall extends Component {
    ...
    componentDidUpdate(prevProps,prevState){
        if(prevState.selectedTimeTable!==this.state.selectedTimeTable){
            let currentRollCall=this.props.rollcalls.rollcalls.find(r=>r.day===this.state.date && r._timeTable.id===this.state.selectedTimeTable.id && r.index===this.state.selectedStudyIndex)
            this.setState({_students:currentRollCall!==undefined?currentRollCall._students:[]},()=>alert(currentRollCall))
        }
    }
    ...
    saveRollCall=()=>{
        ...
        this.state._students.map(item=>studentID.push(item._account.id))
    ...
    sendPushNotification = (_student) => {
        alert(_student._account.push_token)
